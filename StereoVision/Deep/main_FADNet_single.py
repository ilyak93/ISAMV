...
from losses.multiscaleloss import multiscaleloss, EPE, EPE_single, multiscaleloss_single
...
from networks.FADNet_single import FADNet_single
...
if __name__ == '__main__':
    ...
    input_channel = 1
    net = FADNet_single(input_channel=input_channel).cuda()
    ...
    for r in range(startRound, len(FADNet_loss_config["epoches"])):
        ...
        for k in range(cycles):
            ...
            for [left_img, right_img], [target_disp, depths] in train_dataloader:
                ...
                inputs = target_disp.cuda().unsqueeze(1)
                output_net1, output_net2 = net(inputs)
                criterion = multiscaleloss_single(FADNet_loss_config['loss_scale'],
                ...
                flow2_EPE = EPE_single(output_net2_final, target_dis)
                ...
                if i % 10 == 0:
                    ...
                    writer.add_scalar("train/per_10_iterations/loss1", loss_net1.data.item(), i)
                    writer.add_scalar("train/per_10_iterations/loss2", loss_net2.data.item(), i)
            ...
            with torch.no_grad():
                ...
                for [left_img, right_img], [target_disp, depths] in test_dataloader:
                    ...
                    inputs = target_disp.cuda().unsqueeze(dim=1)

                    output_net1, output_net2 = net(inputs)

                    loss_net1 = EPE_single(output_net1, target_dis)
                    loss_net2 = EPE_single(output_net2, target_dis)
                    loss = loss_net1 + loss_net2
                    flow2_EPE = EPE_single(output_net2, target_dis)
                    ...
                    writer.add_scalar("test/per_10_iterations/loss1", loss_net1.data.item(), j)
                    writer.add_scalar("test/per_10_iterations/loss2", loss_net2.data.item(), j)
                    writer.add_scalar("test/per_10_iterations/EPE", flow2_EPE.data.item(), j)
                    if j % 2 == 0:
                        orig_viz = torch.cat((inputs[0].cpu(),
                                              target_dis[0].cpu(),
                                              output_net2[0].cpu(),
                                              ...
                                              output_net2[0].cpu() * max_uint16,
                                              output_net1[0].cpu(),
                                              output_net1[0].cpu() / max_uint16,
                                              output_net1[0].cpu() / 256,
                                              output_net1[0].cpu() * 256,
                                              output_net1[0].cpu() * max_uint16,
                                              ), 0).unsqueeze(1)
