...
if __name__ == '__main__':
    ...
    test_dataloader = DataLoader(val_set, batch_size=batch_size, shuffle=False, num_workers=2)
    ...
    for r in range(startRound, len(FADNet_loss_config["epoches"])):
        ...
        for k in range(cycles):
            ...
            for [left_img, right_img], [target_disp, depths] in train_dataloader:
                ...
                target_disp = target_disp.unsqueeze(dim=1).cuda() / max_uint16
                ...
                flow2_EPE = EPE(output3, target_disp)
                ...
                for [left_img, right_img], [target_disp, depths] in test_dataloader:
                    ...
                    if j % 2 == 0:
                        orig_viz = torch.cat((left_img[0].cpu(),
                                              ...
                                              target_disp[0].cpu(),
                                              output_net3[0].cpu(),
                                              output_net3[0].cpu() / max_uint16,
                                              output_net3[0].cpu() / 256,
                                              output_net3[0].cpu() * 256,
                                              output_net3[0].cpu() * max_uint16
                                              ), 0).unsqueeze(1)
