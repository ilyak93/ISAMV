...
public:
    ...
    char** color_depth_ts_mfd_ptr = new char*;
...
public:
    RSCallback(char* cmfd_ptr, char* dmfd_ptr, char* cdtsmfd_ptr,
               std::atomic<volatile int>& idxc, int& idxd, std::mutex& mx,
               boost::shared_mutex& smx) :
               idx_color(idxc), idx_depth(idxd), mux(mx), shared_mux(smx) {
        *color_mfd_ptr = cmfd_ptr;
        *depth_mfd_ptr = dmfd_ptr;
        *color_depth_ts_mfd_ptr = cdtsmfd_ptr;
        ...
        if (rs2::frameset fs = frame.as<rs2::frameset>()) {
            ...
            for (const rs2::frame f: fs) {
                ...
                if (vf.get_bytes_per_pixel() == 2) {
                    ...
                    size_t ts_sz = sizeof(long long int);
                    const char* loc_ts_bytes = static_cast<const char*>(static_cast<const void*>(&loc_ts));
                    memcpy((void *) ((uint8_t *) (*color_depth_ts_mfd_ptr) +
                    idx_color * ts_sz), loc_ts_bytes, ts_sz);
...
public:
    TC1Callback(char *tcm_ptr, char *ttsm_ptr, int& ixtc,
                size_t tc_size, std::mutex &mx, boost::shared_mutex &smx) :
                idx_tc(ixtc), sz(tc_size),
            mux(mx), shared_mux(smx) {
        *tc_mfd_ptr =  tcm_ptr;
        *tc_ts_mfd_ptr = ttsm_ptr;
    }
    ...
    void operator()(const vector <uint8_t> &cur_frame) {
        ...
        size_t ts_sz = sizeof(long long int);
        const char* loc_ts_bytes = static_cast<const char*>(static_cast<const void*>(&loc_ts));
        memcpy((void *) ((uint8_t *) (*tc_ts_mfd_ptr) +
        idx_tc * ts_sz), loc_ts_bytes, ts_sz);
        idx_tc++;
    }
};
...
public:
    TC2Callback(char *tcm_ptr, char *ttsm_ptr, int& ixtc,
                size_t tc_size, std::mutex &mx, boost::shared_mutex &smx) :
            idx_tc(ixtc), sz(tc_size),
            mux(mx), shared_mux(smx) {
        *tc_mfd_ptr =  tcm_ptr;
...
public:
    ...
    mapped_file& cd_ts_mapped_fd;
    mapped_file& tc1_ts_mapped_fd;
    mapped_file& tc2_ts_mapped_fd;
    int idx;
    string save_dir;

public:
    SaveCallback(mapped_file& cmfd, mapped_file& dmfd, mapped_file& tc1fd,
                 mapped_file& tc2fd, mapped_file& cdtsm,
                 mapped_file& t1tsm, mapped_file& t2tsm,
                 int ix, string sdir) : color_mapped_fd(cmfd),
                 depth_mapped_fd(dmfd), tc1_mapped_fd(tc1fd),
                 tc2_mapped_fd(tc2fd),
                 cd_ts_mapped_fd(cdtsm),
                 tc1_ts_mapped_fd(t1tsm), tc2_ts_mapped_fd(t2tsm), idx(ix),
                 save_dir(sdir) {}
    ...
    void operator()() {
        ...
        tc1_mapped_fd.close();
        tc2_mapped_fd.close();
        cd_ts_mapped_fd.close();
        tc1_ts_mapped_fd.close();
        tc2_ts_mapped_fd.close();
    }
};
...
int main() {
    ...
    long long cd_ts_size = sizeof(long long int) * rs_fps * time_to_record;
    ...
    char **tc1_ts_mfd_ptrs = (char **) new char *[number_of_records];
    mapped_file * tc1_ts_mapped_fds = (mapped_file * ) new mapped_file[number_of_records];

    char **tc2_ts_mfd_ptrs = (char **) new char *[number_of_records];
    mapped_file * tc2_ts_mapped_fds = (mapped_file * ) new mapped_file[number_of_records];
    ...
    for (int l = 0; l < number_of_records; ++l) {
        ...
        string cd_ts_file_path = save_dir + to_string(l) + "cd_ts.bin";
        const char *cd_ts_FileName = cd_ts_file_path.c_str();
        const std::size_t ColorDepthTsFileSize = cd_ts_size;

        mapped_file_params params_cd_ts(cd_ts_FileName);
        params_cd_ts.new_file_size = ColorDepthTsFileSize;
        params_cd_ts.flags = mapped_file_base::readwrite;
        cd_ts_mapped_fds[l] = mapped_file(params_cd_ts);
        cd_ts_mfd_ptrs[l] = cd_ts_mapped_fds[l].data();

        string t1_ts_file_path = save_dir + to_string(l) + "t1_ts.bin";
        const char *t1_ts_FileName = t1_ts_file_path.c_str();
        const std::size_t T1TsFileSize = tc_ts_size;

        mapped_file_params params_t1_ts(t1_ts_FileName);
        params_t1_ts.new_file_size = T1TsFileSize;
        params_t1_ts.flags = mapped_file_base::readwrite;
        tc1_ts_mapped_fds[l] = mapped_file(params_t1_ts);
        tc1_ts_mfd_ptrs[l] = tc1_ts_mapped_fds[l].data();

        string t2_ts_file_path = save_dir + to_string(l) + "t2_ts.bin";
        const char *t2_ts_FileName = t2_ts_file_path.c_str();
        const std::size_t T2TsFileSize = tc_ts_size;

        mapped_file_params params_t2_ts(t2_ts_FileName);
        params_t2_ts.new_file_size = T2TsFileSize;
        params_t2_ts.flags = mapped_file_base::readwrite;
        tc2_ts_mapped_fds[l] = mapped_file(params_t2_ts);
        tc2_ts_mfd_ptrs[l] = tc2_ts_mapped_fds[l].data();
    ...
    auto tc1_callback = TC1Callback(tc1_mfd_ptrs[0], tc1_ts_mfd_ptrs[0], idx_tc1,
    ...
    auto tc2_callback = TC2Callback(tc2_mfd_ptrs[0], tc2_ts_mfd_ptrs[0], idx_tc2,
    ...
    auto rs_callback = RSCallback(color_mfd_ptrs[0], depth_mfd_ptrs[0],
                                  cd_ts_mfd_ptrs[0], idx_color, idx_depth, mux,
                                  shared_mux);
    ...
    for (int cur_idx = 0; cur_idx < number_of_records; ++cur_idx) {
...

        {
            ...
            auto save_callback = SaveCallback(color_mapped_fds[cur_idx],
                                              ...
                                              cd_ts_mapped_fds[cur_idx],
                                              tc1_ts_mapped_fds[cur_idx],
                                              tc2_ts_mapped_fds[cur_idx],
                                              cur_idx, save_dir);
