...
def main():
    for i in range(1, 2):
        g_path = "G:/Vista_project/fusion/" +str(i) + "/"
        ...
        for l, r, lts, rts in zip(left,right, left_ts, right_ts):
            with open(g_path+l, "rb") as left_bytes_stream,\
                    open(g_path+r, "rb") as right_bytes_stream,\
                    open(g_path + lts, "rb") as left_ts_bytes_stream,\
                    open(g_path + rts, "rb") as right_ts_bytes_stream :
        ...
        for color, depth, color_ts in zip(rs_color, rs_depth, rs_color_ts):
            with open(g_path + color, "rb") as color_bytes_stream, \
                    open(g_path + depth, "rb") as depth_bytes_stream, \
                    open(g_path + color_ts, "rb") as color_ts_bytes_stream :
                while (True):
