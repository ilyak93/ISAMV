...
path = "E:/raw_data/bright/"
dirs = os.listdir(path)
dirs = [dir for dir in dirs if "data_2" in dir]
n = 1
dest_path = 'E:/dataset/'
for dir in dirs:
    color_depth_files = os.listdir(path+'/'+dir+"/aligned_rs/")
    color_files = [path+'/'+dir+'/aligned_rs/'+img_file for img_file in color_depth_files if "color" in img_file]
    depth_files = [path+'/'+dir+'/aligned_rs/'+img_file for img_file in color_depth_files if "depth" in img_file]
    left_files = os.listdir(path+'/'+dir+"/left/")
    left_files = [path+'/'+dir+'/left/'+file for file in left_files]
    right_files = os.listdir(path+'/'+dir+"/right/")
    right_files = [path+'/'+dir+'/right/'+file for file in right_files]

    color_files = winsort(color_files)
    depth_files = winsort(depth_files)
    left_files = winsort(left_files)
    right_files = winsort(right_files)
    for cur_file_idx in range(66, len(color_files)):

        np_color_im = cv2.imread(color_files[cur_file_idx], cv2.IMREAD_COLOR)
        np_color_im = cv2.cvtColor(np_color_im, cv2.COLOR_BGR2RGB)
        np_depth_im = cv2.imread(depth_files[cur_file_idx], cv2.IMREAD_ANYDEPTH)
        np_left_im = cv2.imread(left_files[cur_file_idx], cv2.IMREAD_ANYDEPTH)
        np_right_im = cv2.imread(right_files[cur_file_idx], cv2.IMREAD_ANYDEPTH)
        ...
        disparity[igul_u_left[indices], igul_v_left[indices]] = igul_v_left[indices] - igul_v_right[indices]
        # worked disparity[igul_u_left[indices], igul_v_left[indices]] =  igul_v_left[indices] - igul_v_right[indices]
        ...
        #hist(depth, bins='auto')
        #show()
        #imshow(depth, 'gray')
        #show()
        np_left_im = np_left_im[0:512, 0:640]
        np_left_im = Image.fromarray(np_left_im)
        np_left_im.save(dest_path + "/small_correct/" + str(n) + "_left.png", format='PNG')
        np_right_im = np.roll(np_right_im, 14, axis=0)[0:512, 0:640]
        np_right_im[0:14, :] = 0
        np_right_im = Image.fromarray(np_right_im)
        np_right_im.save(dest_path + "/small_correct/" + str(n) + "_right.png", format='PNG')
        disparity = disparity[0:512, 0:640]
        disparity = Image.fromarray(disparity)
        disparity.save(dest_path + "/small_correct/" + str(n) + "_dis.png", format='PNG')
        depth = depth[0:512, 0:640]
        depth = Image.fromarray(depth)
        depth.save(dest_path + "/small_correct/" + str(n) + "_depth.png", format='PNG')
        n = n + 1

        #disparity to depth fb=470
